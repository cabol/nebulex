defmodule Nebulex.Cache.KV do
  ...
  def get(name, key, default, opts) do
    with_meta(name, &do_get(&1, key, default, opts))
  end
  ...
  defp do_put(name, key, value, on_write, opts) do
    ...
    do_put(name, key, value, ttl, on_write, opts)
  end

  @compile {:inline, do_put: 6}
  defcommandp do_put(name, key, value, ttl, on_write, opts), command: :put
  ...
  def do_put_all(name, entries, on_write, opts) do
    ...
    do_put_all(name, entries, ttl, on_write, opts)
  end

  @compile {:inline, do_put_all: 5}
  defcommandp do_put_all(name, entries, ttl, on_write, opts), command: :put_all
  ...
  def incr(name, key, amount, opts) when is_integer(amount) do
    ...
    do_incr(name, key, amount, ttl, default, opts)
  end
  ...
  defcommandp do_incr(name, key, amount, ttl, default, opts), command: :update_counter
  ...
  def get_and_update(name, key, fun, opts) when is_function(fun, 1) do
    ...
    with_meta(name, fn adapter_meta ->
      with {:ok, current} <- do_get(adapter_meta, key, nil, opts) do
        {:ok, eval_get_and_update_function(current, adapter_meta, key, ttl, opts, fun)}
      end
    end)
  end

  defp eval_get_and_update_function(current, adapter_meta, key, ttl, opts, fun) do
    case fun.(current) do
      {get, nil} ->
        {get, get}

      {get, update} ->
        {:ok, true} = run_command(adapter_meta, :put, [key, update, ttl, :put], opts)
  ...
  def update(name, key, initial, fun, opts) when is_function(fun, 1) do
    ...
    with_meta(name, fn adapter_meta ->
      ...
      with {:ok, true} <- run_command(adapter_meta, :put, [key, value, ttl, :put], opts) do
        {:ok, value}
      end
    end)
  catch

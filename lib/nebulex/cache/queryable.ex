defmodule Nebulex.Cache.Queryable do
  ...
  alias Nebulex.Cache.QuerySpec

  # Default max entries to load from the cache as we stream
  ...
  def stream(name, query_spec, opts) do
    opts = Keyword.put_new(opts, :max_entries, @default_max_entries)

    with {:ok, stream} <- do_stream(name, query_meta(query_spec, :stream), opts) do

if Code.ensure_loaded?(Decorator.Define) do
  defmodule Nebulex.Caching.Decorators do
    ...
    strategy given by `Nebulex.Caching.SimpleKeyGenerator`, which is
    ...
    `Nebulex.Caching.KeyGenerator` behaviour. Once configured via the
    `:default_key_generator` option, the generator will be used for each
    declaration that does not specify its own key generation strategy (see the
    ["Custom Key Generation"](#module-custom-key-generation-declaration)
    section down below).
    ...
    The following example shows how to configure a custom default key generator:
        ...
        defmodule MyApp.Books do
          use Nebulex.Caching,
            cache: MyApp.Cache
            default_key_generator: MyApp.Keygen
    ...
    the key is generated through its `:key` attribute (the same applies to all
    ...
    In the last example, an anonymous function is used for generating the key.
    It must be an anonymous function that expects the
    [decorator's context](`t:context/0`) as an argument.

    One can also provide options for the cache commands executed underneath,
    like so:

        @decorate cacheable(cache: Cache, key: isbn, opts: [ttl: 300_000])
        def find_book(isbn, check_warehouse?, include_used?) do
          # the logic for retrieving the book ...
        end

    In that case, `opts: [ttl: 300_000]` specifies the TTL for the cached value.
    ...
    `MyApp.Books`, we may decorate some functions as follows:
        ...
        defmodule MyApp.Books do
          ...
          @decorate cache_evict(keys: [book.id, book.isbn])
    ...
    retrieving the user account by any of those fields, like so:
        ...
        defmodule MyApp.UserAccounts do
          ...
          @decorate cache_evict(keys: [user.id, user.email, user.token])
    ...
    * When the `cache_evict` decorator annotates a key (or keys) to evict, the
    ...
    In contrast to the `cacheable` decorator, this decorator does not cause the
    decorated function to be skipped. Instead, it always causes the function
    to be invoked and its result to be stored in the associated cache if the
    condition given by the `:match` option matches accordingly.

    ## Options

    #{Nebulex.Caching.Options.cache_put_options_docs()}
    ...
    ## Examples
        ...
        defmodule MyApp.Example do
          ...
          @decorate cache_evict(keys: [object.name, object.id])
    ...
    defp caching_action(decorator, attrs, block, context) do
      ...
      caching_opts = Module.get_attribute(context.module, :__caching_opts__, [])

      # Resolve the cache to use
      cache_var = get_cache(attrs, caching_opts)
      ...
      keygen_block = keygen_block(attrs, caching_opts)
      ...
      action_block =
        action_block(
          ...
          attrs,
          keygen_block,
          on_error_opt(attrs, Keyword.fetch!(caching_opts, :on_error)),
          attrs[:match] || default_match_fun()
        )
      ...
      quote do
        ...
        :ok = unquote(__MODULE__).put_decorator_context(unquote(context))
        ...
        after
          ...
          unquote(__MODULE__).del_decorator_context()
        end
      end
    end

    defp get_cache(attrs, caching_opts) do
      attrs[:cache]
      |> Kernel.||(caching_opts[:cache])
      |> Kernel.||(
        raise ArgumentError,
              "expected :cache option to be found within the decorator options " <>
                "if it is not configured globally in the caching definition " <>
                "(e.g., `use Nebulex.Caching, cache: MyCache`)"
      )
    end
    ...
    defp keygen_block(attrs, caching_opts) do
      case Keyword.fetch(attrs, :key) do
        {:ok, key} ->
          quote(do: unquote(key))

        :error ->
          generator = Keyword.fetch!(caching_opts, :default_key_generator)

          quote(do: &unquote(generator).generate/1)
      end
    end
    ...
    defp get_key(attrs, default) do
      with keys when is_list(keys) and length(keys) > 0 <-
             get_option(
               attrs,
               :keys,
    ...
    @compile {:inline, put_decorator_context: 1, get_decorator_context: 0, del_decorator_context: 0}

    @doc """
    Convenience function to set the decorator's context
    for the current process.

    **NOTE:** Internal purposes only.
    """
    @doc group: "Internal API"
    @spec put_decorator_context(context()) :: :ok
    def put_decorator_context(context) do
      _ = Process.put({__MODULE__, :decorator_context}, context)

      :ok
    end

    @doc """
    Convenience function to get the decorator's context
    from the current process.

    **NOTE:** Internal purposes only.
    """
    @doc group: "Internal API"
    @spec get_decorator_context() :: context() | nil
    def get_decorator_context do
      Process.get({__MODULE__, :decorator_context})
    end

    @doc """
    Convenience function to delete the decorator's context
    from the current process.

    **NOTE:** Internal purposes only.
    """
    @doc group: "Internal API"
    @spec del_decorator_context() :: :ok
    def del_decorator_context do
      _ = Process.delete({__MODULE__, :decorator_context})
    ...
    **NOTE:** Internal purposes only.
    ...
    defp do_evict(false, cache, {:"$keys", keys}, on_error) do
    ...
    **NOTE:** Internal purposes only.
    """
    @doc group: "Internal API"
    @spec cache_put(cache_value(), {:"$keys", any()} | any(), any(), keyword()) :: :ok

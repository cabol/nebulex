defmodule Nebulex.Adapters.Nil do
  ...
  @impl true
  def fetch(_adapter_meta, key, _) do
  ...
  @impl true
  def take(_adapter_meta, key, _) do
    wrap_error Nebulex.KeyError, key: key
  end

  @impl true
  def has_key?(_, _, _), do: {:ok, false}

  @impl true
  def ttl(_adapter_meta, key, _opts) do
    wrap_error Nebulex.KeyError, key: key
  end
  ...
  @impl true
  def execute(_, %{op: :get_all}, _), do: {:ok, []}
  def execute(_, _, _), do: {:ok, 0}

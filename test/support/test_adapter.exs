defmodule Nebulex.TestAdapter do
  ...
  defp validate_ttl(:error, key, _) do
  ...
  def stream(%{pid: pid}, query_meta, opts) do
    ...
    with :ok <- assert_query(query_meta.query) do
      GenServer.call(pid, {:q, query_meta, max_entries})
    end
  end
...
defmodule Nebulex.TestAdapter.KV do
  ...
  def handle_call(
        {:q, %{op: :stream, query: {:q, nil}, select: select}, max_entries},
        _from,
        %__MODULE__{map: map} = state
      ) do
    {:reply, {:ok, stream_unexpired(map, max_entries, select)}, state}
  end

  def handle_call(
        {:q, %{op: :stream, query: {:in, keys}, select: select}, max_entries},
        _from,
        %__MODULE__{map: map} = state
      ) do
    now = Time.now()
    ...
    stream =
      map
      |> Stream.filter(fn {k, %Entry{exp: exp}} ->
  ...
  defp stream_unexpired(map, max_entries, select) do
    now = Time.now()

    map
    |> Stream.filter(fn {_, %Entry{exp: exp}} -> exp > now end)
    |> return(Stream, select)
    |> Stream.chunk_every(max_entries)
  end
